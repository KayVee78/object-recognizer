...
if isfield(handles, 'ImageData') && ~isempty(handles.ImageData)
    ...
    % Apply elliptical mask to the enhanced image to isolate the tumor area
    % Logical operation ensures the result is a binary image
    tumorMask = logical(enhancedImg .* mask);
    % Filtering connected components based on size
    tumorMask = bwareafilt(tumorMask, [700 4000]);

    % Perform morphological closing to smoothen the boundaries and fill small gaps
    maskStructuring = strel('disk', 6);
    tumorMask = imclose(tumorMask, maskStructuring);
    % Fills the remaining holes
